...
// Middleware to log incoming requests with all required fields
app.use((req, res, next) => {
  ...
  // Log details in the desired structure
  logger.info({
    endpoint: req.url, // Log the requested endpoint
    ip: ip, // Log the IP address
    level: "info", // Log level
    message: "Incoming request", // Custom message
    method: req.method, // HTTP method (GET, POST, etc.)
    queryParams: req.query, // Log the query parameters if any
    requestBody: req.body, // Log the request body
    timestamp: timestamp, // Log the timestamp of the request
...
app.get("/debug/questions", (req, res) => {
  ...
  fs.readdir(questionsDir, (err, files) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Unable to read questions directory." });
    }
    res.json({ files });
  });
});
...
app.get("/v1/:languageCode/love-language", (req, res) => {
  ...
  try {
    ...
    res
      .status(404)
      .json({ error: `Questions not found for language: ${languageCode}` });
  }
});
...
app.post("/v1/:languageCode/submit-love-language-answers", (req, res) => {
  ...
  try {
    ...
    const detailsFilePath = path.join(
      __dirname,
      "answers",
      `loveLanguage_${languageCode}.json`
    );
    ...
    try {
      ...
      logger.error(
        `Error loading or parsing file for language ${languageCode}:`,
        error
      );
      return res
        .status(400)
        .json({ error: `Language ${languageCode} is not supported.` });
    }
    ...
    answers.forEach(({ questionId, answer }) => {
      ...
      if (loveLanguage) {
        ...
        const englishStyle = Object.keys(loveLanguageTranslations["en"]).find(
          (key) =>
            loveLanguageTranslations[languageCode][key].toLowerCase() ===
            loveLanguage.toLowerCase()
        );
        if (englishStyle) {
          score[englishStyle]++;
        } else {
          logger.warn(
            `No mapping found for style ${loveLanguage} in language ${languageCode}`
          );
        }
      } else {
        logger.warn(
          `No mapping found for question ${questionId} and answer ${normalizedAnswer}`
        );
      }
    });
    ...
    const translatedStyles =
      loveLanguageTranslations[languageCode] || loveLanguageTranslations["en"];

    // Create the ranked response including all the details
    const rankedDetails = ranked.map(([style, score], index) => {
      const translatedStyle = translatedStyles[style]
        .replace(/[’']/g, "")
        .toLowerCase();
...
app.post("/v1/:languageCode/submit-conflict-answers", (req, res) => {
  ...
  try {
    ...
    // Get the appropriate mapping based on the languageCode
    const conflictManagementMapping = require("./conflictManagementMapping")(
      languageCode
    );
    logger.info(`Loaded mapping for language: ${languageCode}`);
    ...
    const detailsFilePath = path.join(
      __dirname,
      "answers",
      `conflictManagement_${languageCode}.json`
    );
    logger.info(`Loading file from: ${detailsFilePath}`);
    ...
    try {
      ...
      logger.info(
        `Loaded conflict management details for language: ${languageCode}`
      );
    } catch (error) {
      logger.error(
        `Error loading or parsing file for language ${languageCode}:`,
    ...
    answers.forEach(({ questionId, answer }) => {
      ...
      const conflictStyle =
        conflictManagementMapping[questionId][normalizedAnswer]; // Look up the correct style from mapping
      ...
      if (conflictStyle) {
        ...
        if (englishStyle) {
          ...
          logger.warn(
            `No mapping found for style ${conflictStyle} in language ${languageCode}`
          );
        }
      } else {
        logger.warn(
    ...
    // Translate the styles from English to the appropriate language
    const translatedStyles =
      styleTranslations[languageCode] || styleTranslations["en"];
    ...
    const rankedDetails = ranked.map(([style, score], index) => {
      ...
      if (!styleDetails) {
        return {
          ...
          score,
          error: `Details for style ${
            translatedStyle || style
